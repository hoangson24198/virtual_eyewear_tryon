...
class PnpHeadPoseEstimator:
    ...
    def __init__(self, cam_w, cam_h, assets=None):
        ...
        if (DEBUG == True): print(
            "[DEEPGAZE] PnpHeadPoseEstimator: estimated camera matrix: \n" + str(self.camera_matrix) + "\n")
        # load face models
    ...
    def return_roll_pitch_yaw_slow(self, landmarks, cam_w, cam_h):
        ...
        (mesh, pose, shape_coeffs, blendshape_coeffs) = eos.fitting.fit_shape_and_pose(
            self.morphablemodel_with_expressions, landmarks_new, self.landmark_mapper, cam_w, cam_h, self.edge_topology,
            self.contour_landmarks, self.model_contour, num_iterations=1)
        # PYTHON2.7
